...
<section id="services" class="services service-section">
  ...
  if  (isset($_POST['sortbirth'])) {
	   sortbirth();
	   }
  if  (isset($_POST['sortvisitdate'])) {
	   sort_visitdate();
	   }
...
_END;
  ...
  exit;
	 ...
	 function sortfname(){
		...
		$conn = new mysqli($hn, $un, $pw, $db);
		if ($conn->connect_error) die($conn->connect_error);
		$query = "SELECT * FROM patient p JOIN demographic d on p.p_id = d.P_id ORDER BY p.`p_firstname`  DESC";
		  $result = $conn->query($query);
			if (!$result) die($conn->error);
			$rows = $result->num_rows;
...
_END;
	 ...
	 function sortbirth(){
		...
		$conn = new mysqli($hn, $un, $pw, $db);
		if ($conn->connect_error) die($conn->connect_error);
		$query = "SELECT * FROM patient p JOIN demographic d on p.p_id = d.P_id ORDER BY d.birth_date  DESC";
		  $result = $conn->query($query);
			if (!$result) die($conn->error);
			$rows = $result->num_rows;
...
_END;
	  ...
	  function sort_visitdate(){
		require_once 'connect_db.php';               ##Database Connection
		$conn = new mysqli($hn, $un, $pw, $db);
		if ($conn->connect_error) die($conn->connect_error);
		$query = "SELECT * FROM patient p JOIN demographic d on p.p_id = d.P_id ORDER BY d.rec_visit_date DESC";
		  $result = $conn->query($query);
			if (!$result) die($conn->error);
			$rows = $result->num_rows;
